...
def get_expenses(user, start_date, end_date):
    ...
    try:
        expenses_data = Expense.query.filter(
            Expense.expense_date.between(f'{start_date}', f'{end_date}')
        )
        if not expenses_data:
            ...
            """)
        expenses = expenses_schema.dump(expenses_data)
        return custom_make_response("data", expenses, 200)
...
def update_expense(user, id):
    ...
    try:
        ...
        expense = Expense.query.filter_by(expense_sys_id=id).first()
        if not expense:
            abort(404, "The expense you are updating does not exist")

        Expense.query.filter_by(expense_sys_id=id).update(
...
def delete_expense(user, id):
    ...
    try:
        ...
        Expense.query.filter_by(expense_sys_id=id).delete()
        db.session.commit()
        return custom_make_response(
